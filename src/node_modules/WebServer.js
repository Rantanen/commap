...
WebServer.prototype.suggestTarget = function (client, markerId, target) {
    client.broadcast.suggestTarget( markerId, target, client.name );
};

WebServer.prototype.rejectSuggestion = function (client, markerId, playerName) {
    client.broadcast.rejectSuggestion( markerId, playerName );
};

WebServer.prototype.ping = function (client) {
    if( !client.name ) {
        log( "Ping from a non-player client: " + client.id );
        return;
    }

    var timer = this.playerTimers[ client.name ];
    if( timer ) {
        clearTimeout( timer );
    }
    ...
    this.playerTimers[ client.name ] = setTimeout( function () {
        log( 'Ping timeout: ' + name );
